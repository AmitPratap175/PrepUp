...
const OdishaTopicwisePracticePage: React.FC = () => {
    ...
    const convertToPracticeTest = (data: QuizData, index: number): PracticeTest => {
        ...
        const testId = `odisha_cw_${index}_${safeTitle}`;
        ...
        // Heuristic to clean subject
        let subject = data.subject || "General Odisha";
        subject = subject.replace(/\w\S*/g, (txt) => txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase());

        return {
            id: testId,
            title: data.title || `Odisha Chapter ${index + 1}`,
            examType: "opsc",
            subject: subject,
    ...
    const fetchQuizzes = async () => {
        try {
            setLoading(true);
            const response = await fetch('/data/odisha_mcqs.json');
    ...
    const handleSelectQuiz = (testId: string) => {
        ...
        const stored = localStorage.getItem(`odisha_result_${testId}`);
        if (stored) {
    ...
    const handleQuizSubmit = (answers: UserAnswer[]) => {
        ...
        if (activeTest) {
            ...
            localStorage.setItem(`odisha_result_${activeTest.id}`, JSON.stringify(result));
    ...
    const handleExportSubjectPDF = async (subject: string, tests: PracticeTest[]) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `Odisha_Practice_${subject}.pdf`;
                a.click();
            } else {
                throw new Error("Export failed");
            }
                                                        ...
                                                        <CardHeader className="flex-none pb-2">
                                                            <div className="flex items-start justify-between gap-4">
                                                                <div className="flex items-start gap-3">
                                                                    ...
                                                                    <div>
                                                                        <CardTitle className="text-lg font-bold leading-tight mb-1 line-clamp-2" title={t.title}>
                                        ...
                                        <CardHeader>
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <CardDescription>Comprehensive Odisha State Practice</CardDescription>
                                                </div>
...
export default OdishaTopicwisePracticePage;
