...
![](https://epaper.thehindu.com/ccidist-ws/th/th_international/issues/147382/OPS/Public/GT6ERD62F.1+GTDESEJK8.1.jpg)
...
export default function CurrentAffairsArticlePage() {
  ...
  // For now, only article 1 is supported
  const articleContent = articleId === '1' ? article1Content : "Article not found";

  const numericArticleId = Number(articleId);
  const prevArticleId = numericArticleId > 1 ? numericArticleId - 1 : null;
  const nextArticleId = numericArticleId < 23 ? numericArticleId + 1 : null;
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-8">
          <h1 className="text-3xl font-bold mb-4">Current Affairs: Article {articleId}</h1>
          <div className="prose max-w-none text-foreground leading-relaxed preserve-whitespace">
            <Latex>{articleContent}</Latex>
          </div>
          <div className="flex justify-between mt-8">
            {prevArticleId && (
              <Link href={`/current-affairs/${prevArticleId}`}>
                <Button>Previous Article</Button>
              </Link>
            )}
            {nextArticleId && (
              <Link href={`/current-affairs/${nextArticleId}`}>
