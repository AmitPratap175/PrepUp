...
INPUT_PDF = Path("/home/dspratap/Downloads/PrepUp/UPSC/NCERT/Constitutional Articles/india_constitution_all_articles.pdf")
PROCESSED_LOG_PATH = DATA_OUT_DIR / "processed_constitution_pages.json"
...
# --- Prompts ---
PROMPT_REMEMBERING = """
Act as an expert educator. Your goal is to help me revise and memorize the specific facts in the uploaded constitution document page.

**MANDATORY REQUIREMENTS:**
1. **FOCUS:** Generate direct, factual questions testing memory of Article numbers, Parts of the Constitution, specific clauses, and key terms mentioned *exactly* as they appear on this page.
2. **QUANTITY:** Exhaustively cover all facts on this page. Do not leave a single article or clause untested.
...
**MANDATORY REQUIREMENTS:**
1. **UPSC FORMATS:** Generate Statement-Based Traps, Chronology, and Match the Following style questions.
...
3. **QUANTITY:** Generate diverse, hard difficulty questions covering the nuances of the articles on this page.
    ...
    try:
        ...
        if temp_file.exists():
            with open(temp_file, 'r') as f:
                data = json.load(f)
            # NotebookLM exports structure as { title, questions: [] } mostly
            qs = data.get("questions", [])
            print(f"    - [{prompt_type}] Generated {len(qs)} questions.")
            
            # Clean up explicit temp file
            temp_file.unlink()
...
async def main():
    ...
    async with await NotebookLMClient.from_storage(STORAGE_PATH) as nb_client:
        for i in range(total_pages):
            ...
            # Extract the single page into a temporary PDF resource explicitly
            writer = PdfWriter()
            writer.add_page(reader.pages[i])
            
            temp_pdf_path = DATA_OUT_DIR / f"temp_page_{page_num}.pdf"
            with open(temp_pdf_path, 'wb') as f:
                writer.write(f)
            
            nb = None
            try:
                nb_title = f"Constitution Page {page_num}"
                ...
                # Execute NotebookLM generation sequentially on the new notebook's sources
                remembering_qs = await generate_questions(
                    nb_client, nb.id, PROMPT_REMEMBERING, "Remembering", f"temp_rmb_{page_num}"
                )
                upsc_qs = await generate_questions(
                    nb_client, nb.id, PROMPT_UPSC, "UPSC-Style", f"temp_upsc_{page_num}"
                )
                ...
                # Process IDs and enforce structure integration
                title = f"Constitution of India - Articles (Page {page_num})"
                subject = "Constitutional Articles"
                
                for q_idx, q in enumerate(combined_qs, 1):
                    unique_id = f"const-pg{page_num}-{q_idx}"
                    q['id'] = unique_id
                    q['qid'] = unique_id
                    q['type'] = 'mcq'
                ...
                # Cleanup temporal files & API notebooks
