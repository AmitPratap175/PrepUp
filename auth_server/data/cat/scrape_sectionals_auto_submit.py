...
async def test_news_crawl_submit(urls) -> Dict[str, List[str]]:
    """
    Crawls a predefined list of URLs to fetch quiz pages.
    """
    user_data_dir = os.path.join(Path.home(), ".crawl4ai", "browser_profile")
    ...
    run_config = CrawlerRunConfig(
        scan_full_page=True,
        js_code=[
            "document.getElementById('cracku-interface')?.click()",
            "Array.from(document.querySelectorAll('*')).find(el => el.textContent.includes('START MOCK'))?.click()",
            "setTimeout(() => { Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Submit') && !b.closest('nav'))?.click() }, 5000)",
            "setTimeout(() => { document.getElementById('submit-test')?.click() }, 10000)",
            "setTimeout(() => { Array.from(document.querySelectorAll('button')).find(b => b.textContent.includes('Show Correct Answer'))?.click() }, 15000)",
        ],
...
if __name__ == "__main__":
    ...
    with open(Path(__file__).parent /"urls_cracku_sectionals.txt", "r") as f:
        ...
        for url_curr in urls_raw:
            categorized_html = asyncio.run(test_news_crawl_submit([url_curr]))
            ...
            print("Sleeping for 5 minutes to allow submission processing...")
            time.sleep(5*60)
