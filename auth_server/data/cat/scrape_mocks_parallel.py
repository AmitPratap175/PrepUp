...
async def crawl_urls_parallel(urls: List[str], type_curr: str) -> Dict[str, List[str]]:
    """
    Crawls a list of URLs in parallel using crawl4ai.
    """
    print(f"Starting parallel crawl for {len(urls)} URLs...")
    ...
    browser_config = BrowserConfig(
        verbose=True,
        headless=False, # As per original script
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        for i, result in enumerate(results):
            if result and result.html:
                categorized_html[type_curr].append(result.html)
                # print(f"Successfully crawled URL {i+1}/{len(urls)}")
            else:
                print(f"Failed to crawl URL {i+1}: {urls[i]}")
                
    return categorized_html


# --- Parsing Logic (Quants) ---
...
def main_quants(categorized_html: Dict[str, List[str]], type_curr: str):
    """Main function orchestrating the data processing workflow for Quants."""
...
def main(categorized_html: Dict[str, List[str]], type_curr: str):
    """Main function orchestrating the data processing workflow for VARC/DILR."""
...
if __name__ == "__main__":
    ...
    print("--- Starting PARALLEL daily varc and dilr question processing ---")
    ...
    with open(Path(__file__).parent /"urls_cracku_mocks.txt", "r") as f:
        ...
        for url_curr in urls_raw:
            ...
            # Note: Removed the dummy insertion logic as we are crawling parallelly and directly
            
            # Step 2: Crawl the web pages to get HTML content (Parallel)
            categorized_html = asyncio.run(crawl_urls_parallel(urls, type_curr))
