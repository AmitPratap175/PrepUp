...
telegram_groups = {
    ...
    "The hindu epapers2": 2608082423,
    "atomic habits [ books ]": 1645328529,
    "Saved Messages": "me",  # Special entity for Saved Messages
}
...
async def main():
    ...
    # Print your own user ID (= Saved Messages ID)
    me = await client.get_me()
    print(f"✅ Logged in as: {me.first_name} (ID: {me.id})")
    print(f"📌 Saved Messages entity: 'me' (or user ID {me.id})\n")

    print("📋 Fetching all dialogs (groups, channels, and chats)...\n")
    async for dialog in client.iter_dialogs():
        entity = dialog.entity

        # Saved Messages: your own user entity has is_self = True
        if getattr(entity, 'is_self', False):
            print(f"Saved Messages: {getattr(entity, 'first_name', 'Me')} -> ID: {entity.id}")
            continue
    ...
    print("\n✅ Done listing all groups, channels, and Saved Messages.")

    # --- Scrape URLs from "The hindu epapers2" ---
...
async def scrape_saved_messages(limit=500):
    """Scrape all messages from Saved Messages."""
    await client.start(phone=phone_number)

    data = []
    output_json = "saved_messages.json"

    print(f"\n📥 Fetching up to {limit} messages from Saved Messages...\n")

    async for message in client.iter_messages("me", limit=limit):
        msg_data = pretty_message(message)

        # Also capture any webpage URLs in Saved Messages
        if message.media and hasattr(message.media, "webpage") and message.media.webpage:
            msg_data["url"] = message.media.webpage.url

        # Capture file/document info if present
        if message.document:
            msg_data["document"] = {
    ...
    print(f"✅ Saved {len(data)} messages from Saved Messages to {output_json}")
    return data
...
with client:
    ...
    client.loop.run_until_complete(scrape_saved_messages(limit=500))
    client.loop.run_until_complete(scrape_vocab())

# Convert vocab quizzes to question JSON
