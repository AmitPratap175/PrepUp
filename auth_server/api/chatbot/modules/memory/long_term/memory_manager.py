...
class MemoryManager:
    ...
    def get_relevant_memories(self, context: str, phone_number: str, source: str = "conversation") -> List[str]:
        """Retrieve relevant memories based on the current context."""
        memories = self.vector_store.search_memories(
            context,
            k=settings.MEMORY_TOP_K,
            filter={"must": [{"key": "source", "match": {"value": source}}, {"key": "phone_number", "match": {"value": phone_number}}]},
        )
        if memories:
