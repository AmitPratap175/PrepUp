...
from .tools.content import find_study_materials

class ContentAgentState(TypedDict):
    messages: Annotated[Sequence[BaseMessage], operator.add]

tools = [find_study_materials]
...
prompt_template = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are a helpful assistant who helps users find study materials.",
        ),
        MessagesPlaceholder(variable_name="messages"),
    ]
)

def agent_node(state: ContentAgentState):
