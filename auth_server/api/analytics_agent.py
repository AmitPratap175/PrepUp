...
from .tools.analytics import get_user_performance_summary

class AnalyticsAgentState(TypedDict):
    messages: Annotated[Sequence[BaseMessage], operator.add]

tools = [get_user_performance_summary]
...
prompt_template = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            "You are a helpful assistant who analyzes a user's performance data and provides insights.",
        ),
        MessagesPlaceholder(variable_name="messages"),
    ]
)

def agent_node(state: AnalyticsAgentState):
