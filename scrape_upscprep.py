...
# -*- coding: utf-8 -*-
"""
Combined script for Cracku Daily Test and Solutions crawling.
Successively implements and runs:
1. clean_cracku_dt.py (Questions)
2. clean_cracku_solutions.py (Solutions)
"""
import json
import os
import re
...
# Assuming file_clean_qid.py is in the same directory.
...
def _collect_paragraph_text_simple_dt(container) -> str:
    ...
    ps = [_norm_ws_dt(x) for x in ps if _norm_ws_dt(x)]
    if ps: return "\n\n".join(ps)
    return _norm_ws_dt(container.get_text(" ", strip=True))

def _collect_paragraph_text_dt(container) -> str:
...
def _find_passage_and_question_blocks_dt(qroot, qid: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if blocks:
        ...
        passage_text = _collect_paragraph_text_simple_dt(p_body if p_body else p_block)
        q_block = blocks[-1]
        q_text_div = q_block.find("div", class_=re.compile(r"\bquestion-text\b"))
        if q_text_div:
            question_text = _collect_paragraph_text_dt(q_text_div)
        else:
...
def _extract_correct_answer_dt(qroot) -> Tuple[List[Dict], Optional[str]]:
    ...
    for i, btn in enumerate(buttons, start=1):
        ...
        text = _collect_paragraph_text_dt(content_div) if content_div else None
        if label and text:
...
def _build_full_markdown_dt(qid: str, passage_text: str, question_text: str, options: List[Dict], correct_option_data: Optional[str]) -> str:
...
def parse_html_to_questions_dt(html: str) -> Dict:
    ...
    for qroot in q_roots:
        ...
        if not qid: continue
        passage_text, question_text = _find_passage_and_question_blocks_dt(qroot, qid)
        if not question_text:
            q_text_div = qroot.find("div", class_=re.compile(r"\bquestion-text\b"))
            if q_text_div: question_text = _norm_ws_dt(q_text_div.get_text(" ", strip=True))
        img_tag = qroot.find("img", class_="img-responsive")
        if img_tag and img_tag.has_attr("src"): image_url = img_tag["src"].strip()
        options, correct_option_data = _extract_correct_answer_dt(qroot)
        ...
        if question_text:
            ...
            results.append({"qid": str(qid), "passage_text": passage_text if q_text not in p_text else "", "question_text": question_text or "", "options": options, "correct_option_data": str(correct_option_data) if correct_option_data is not None else None, "solution_text": None, "image_url": image_url or "", "full_markdown": _build_full_markdown_dt(str(qid), passage_text or "", question_text or "", options, correct_option_data)})
    return {"questions": results}

async def crawl_dt(dt_num: int, daily_num: int) -> Dict[str, List[str]]:
    """
    Crawls a predefined list of URLs to fetch quiz pages (DT logic).
    """
    ...
    run_config = CrawlerRunConfig(
        ...
        delay_before_return_html=200.0,
    )
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        urls = [
                "https://courses.upscprep.com/exams/review/34512/answers/"
                ]
...
def main_dt(categorized_html: Dict[str, List[str]]):
    """Main function orchestrating the data processing workflow (DT logic)."""
    ...
    for category, subject_alias in strings.items():
        ...
        for i, html in enumerate(html_docs):
            try:
                ...
                parsed = parse_html_to_questions_dt(html)
                if parsed is None:
...
# --- SOLUTIONS SPECIFIC FUNCTIONS ---

def _norm_ws_sol(s: str) -> str:
    """Normalize whitespace, replacing multiple spaces/newlines with a single space."""
    return re.sub(r"\s+", " ", s).strip()


def _collect_paragraph_text_simple_sol(container) -> str:
    """Extracts and concatenates text from all <p> tags within a given element."""
    if container is None: return ""
    ps = [t.get_text(" ", strip=True) for t in container.find_all("p")]
    ps = [_norm_ws_sol(x) for x in ps if _norm_ws_sol(x)]
    if ps: return "\n\n".join(ps)
    return _norm_ws_sol(container.get_text(" ", strip=True))

def _collect_paragraph_text_sol(container) -> str:
    """Extracts and concatenates text from all <p> tags within a given element, converting katex to latex."""
    if container is None: return ""
...
def _find_passage_and_question_blocks_sol(qroot, qid: str) -> Tuple[Optional[str], Optional[str]]:
    """Heuristically finds the passage and question text within a question's HTML block."""
    passage_text = None
    question_text = None
    en_class = f"en{qid}"
    ...
    if blocks:
        p_block = blocks[0]
        p_body = p_block.find("div", class_=re.compile(r"\bcard-body\b"))
        passage_text = _collect_paragraph_text_simple_sol(p_body if p_body else p_block)
        q_block = blocks[-1]
        q_text_div = q_block.find("div", class_=re.compile(r"\bquestion-text\b"))
        if q_text_div:
            question_text = _collect_paragraph_text_sol(q_text_div)
        else:
            q_text_div = qroot.find("div", class_=re.compile(r"\bquestion-text\b"))
            if q_text_div:
                question_text = _collect_paragraph_text_sol(q_text_div)
...
def _extract_correct_answer_sol(qroot) -> Tuple[List[Dict], Optional[str]]:
    """Extracts all options and identifies the correct one from the HTML."""
    options = []
    ...
    for i, btn in enumerate(buttons, start=1):
        ...
        label = label_tag.get_text(strip=True) if label_tag else None
        content_div = btn.select_one(".option-content")
        text = _collect_paragraph_text_sol(content_div) if content_div else None
        if label and text:
...
def _build_full_markdown_sol(qid: str, passage_text: str, question_text: str, options: List[Dict], correct_option_data: Optional[str]) -> str:
    """Constructs a markdown string for a given question for easy review."""
    lines = [f"### Question (qid: {qid})", "", "**Passage:**", passage_text or "", "", question_text or "", "", "**Options:**"]
    for opt in options:
        mark = " ✅" if opt.get("is_correct") else ""
...
def parse_html_to_questions_sol(html: str) -> Dict:
    """Main parsing function to convert a single HTML page into structured question data."""
    soup = BeautifulSoup(html, "html.parser")
    image_url = None
    results: List[Dict] = []
    ...
    for qroot in q_roots:
        ...
        if qid.lower().startswith("q"):
            ...
            if digits: qid = digits[0]
        if not qid: continue
        passage_text, question_text = _find_passage_and_question_blocks_sol(qroot, qid)
        if not question_text:
            q_text_div = qroot.find("div", class_=re.compile(r"\bquestion-text\b"))
            if q_text_div: question_text = _norm_ws_sol(q_text_div.get_text(" ", strip=True))
        img_tag = qroot.find("img", class_="img-responsive")
        if img_tag and img_tag.has_attr("src"): image_url = img_tag["src"].strip()
        options, correct_option_data = _extract_correct_answer_sol(qroot)
        if question_text is None: continue
        if question_text:
            results.append({"qid": str(qid), "passage_text": passage_text if question_text not in passage_text else "", "question_text": question_text or "", "options": options, "correct_option_data": str(correct_option_data) if correct_option_data is not None else None, "solution_text": None, "image_url": image_url or "", "full_markdown": _build_full_markdown_sol(str(qid), passage_text or "", question_text or "", options, correct_option_data)})
    return {"questions": results}

async def crawl_sol(dt_num: int, daily_num: int) -> Dict[str, List[str]]:
    """
    Crawls a predefined list of URLs to fetch quiz pages (Solutions logic).
    """
    user_data_dir = os.path.join(Path.home(), ".crawl4ai", "browser_profile")
    os.makedirs(user_data_dir, exist_ok=True)
...
def main_sol(categorized_html: Dict[str, List[str]]):
    """Main function orchestrating the data processing workflow (Solutions logic)."""
    strings = {"quant":"quantitative-aptitude", "verbal":"verbal-ability", "reasoning":"data-interpretation"}
    ...
    for category, subject_alias in strings.items():
        ...
        for i, html in enumerate(html_docs):
            try:
                name = f"crawled_{category}_{i}"
                parsed = parse_html_to_questions_sol(html)
                if parsed is None:
                    sys.stderr.write(f"WARNING: No questions parsed from {name}\n")
                    continue
                all_questions.extend(parsed.get("questions", []))
        ...
        if final_path_file.exists():
            ...
            n = len(all_questions)
            for i in range(n):
                # Safely update using index from end, assuming match
                if i < len(final_data["questions"]): # Basic bounds check although logic assumes exact match with recent append
                     # However, logic in original file was:
                     # final_data["questions"][-n+i]["..."] = ...
                     # This assumes the questions were ALREADY appended by the previous run (DT run).
                     # DT run appends new questions. Solutions run updates them.
...
if __name__ == "__main__":
    print("--- Starting daily question processing (DT Phase) ---")
    ...
    categorized_html_dt = asyncio.run(crawl_dt(dt_num, daily_num))
    print(f"Crawled {len(categorized_html_dt['quant'])+len(categorized_html_dt['verbal'])+len(categorized_html_dt['reasoning'])} pages successfully.")
    
    # Step 3: Pass the crawled data to the main processing function.
    main_dt(categorized_html_dt)

    print("\n--- Starting daily question processing (Solutions Phase) ---")
    # Step 1: Get the updated daily numbers for crawling.
    dt_num, daily_num = get_updated_numbers()
    
    # Step 2: Crawl the web pages to get HTML content.
    categorized_html_sol = asyncio.run(crawl_sol(dt_num, daily_num))
    print(f"Crawled {len(categorized_html_sol['quant'])+len(categorized_html_sol['verbal'])+len(categorized_html_sol['reasoning'])} pages successfully.")
    
    # Step 3: Pass the crawled data to the main processing function.
    main_sol(categorized_html_sol)
